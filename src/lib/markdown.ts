...
import type { MarkdownFormat } from "./store";

function createMdInstance(format: MarkdownFormat): MarkdownIt {
  const options: Options = {
    html: true,
    linkify: true,
    typographer: format !== "plain",
  };

  if (format === "commonmark") {
    options.html = false;
    options.typographer = false;
...
const mdInstances: Partial<Record<MarkdownFormat, MarkdownIt>> = {};

function getMdInstance(format: MarkdownFormat): MarkdownIt {
  if (!mdInstances[format]) {
    const md = createMdInstance(format);
    md.use(taskLists, { enabled: true, label: true, labelAfter: true });
    registerPlugins(md, format);
    mdInstances[format] = md;
  }
  return mdInstances[format]!;
}

function registerPlugins(md: MarkdownIt, format: MarkdownFormat) {
  if (format !== "plain") {
...
export function renderMarkdown(
  ...
  format: MarkdownFormat = "pandoc"
): string {
  return getMdInstance(format).render(src, env || {});
}
